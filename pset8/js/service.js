...
function chart()
{
    ...
    for (var i = 0; i < shuttle.seats.length; i++)
    {
        ...
        {
            html += "<li class=" + shuttle.seatscolor[i] + ">" + shuttle.seats[i] + shuttle.seatsdest[i] + "</li>";
        }
    }
    html += "</ol>";
    $("#chart").html(html);
...
{
    var hodla = 0;
    for (e in shuttle.seats)
    {
        if (shuttle.seats[e] != null)
        {
            ...
            var d = shuttle.distance(HOUSES[shuttle.seatsdest[e]].lat, HOUSES[shuttle.seatsdest[e]].lng);
            console.log(d);
            if (Math.floor(d) < 30)
            {
                shuttle.seats[e] = null;
                shuttle.seatsdest[e] = null;
                shuttle.seatscolor[e] = null;
                hodla = 1;
    ...
    if (hodla == 0)
    {
        $("#announcements").html("shuttle not within 30m of any destination");
    }
    chart();
}
...
function pickup()
{
    ...
    {  
        ...
        var d = shuttle.distance(ppass[e].getGeometry().getLatitude(), ppass[e].getGeometry().getLongitude());
       
        if (d < 15)
        {
            var stz = 0;
            if (HOUSES[PASSENGERS[e].house] != null)
            {
                ...
                for (var i = 0; i < shuttle.seats.length; i++)
                {
                    if (shuttle.seats[i] == null)
                    {
                        if (PASSENGERS[e].pickedUp == false)
                        {
                            shuttle.seats[i] = PASSENGERS[e].name;
                            PASSENGERS[e].pickedUp = true;
                            //console.log(PASSENGERS[e].house);
                            shuttle.seatsdest[i] = PASSENGERS[e].house;
                            //console.log(shuttle.seatsdest[i]);
                            shuttle.seatscolor[i] = HOUSES[PASSENGERS[e].house].color;
                            //3D
                            holda = 1;
                            var features = earth.getFeatures();
                            features.removeChild(ppass[e]);
                            //2D
                            mpass[e].setMap(null);
    ...
    if (holda == 0)
    {
        $("#announcements").html("No passenger within 15 meters");
    }
    $("#seats").html(chart());
}
...
function populate()
{
    ...
    for (var i = 0; i < PASSENGERS.length; i++)
    {
        ...
        if (building == PASSENGERS[i].house)
        {
            //nope no populate at own house plz
            i -= 1;
        }
